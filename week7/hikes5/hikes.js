...
class Comment {
  constructor(id, content) {
    ...
    this.date = new Date();
    if (id == 1) {
      this.name = "Bechler Falls";
    }
    else if(id == 2) {
      this.name = "Teton Canyon";
    }
    else {
      this.name = "Dedanda Falls";
    }
  }
}
let comments = [];
...
export default class Hikes {
  ...
  load() {
    alert('inside load');
    let listHike = JSON.parse(localStorage.getItem('hikes'));
    let string = "";
    listHike.forEach(
      item => {
        string += '<li>' + item.name + ' edited: ' + item.date  + '<br>' + item.content + '</li>';
...
<div>
        ...
        <div>
          <textarea id=\'${hike.id}\'></textarea>
          <button id=\'button${hike.id}\'>Change</button>
        </div>
</div>`;
...
function save() {
  let toSave = JSON.stringify(comments);
  localStorage.setItem('hikes', toSave);
}
function addComment(){
