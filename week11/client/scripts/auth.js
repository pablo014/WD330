    ...
    constructor() {
      ...
      async login(callback) {
        ...
        try {
          ...
          this.jwtToken = await makeRequest('login', 'POST', postData)
      ...
      async getCurrentUser(email) {
        try {
          ...
          this.user = await makeRequest('login', 'GET', email, jwtToken)
...
document.getElementById('submit').addEventListener('click', function() {
    let auth = new Auth()
    auth.login()
    //auth.getCurrentUser(document.getElementById('username'))
})
